...
from handanim.stylings.fillpatterns import get_filler
...
class Polygon(Drawable):
    ...
    def draw(self) -> OpsSet:
        ...
        if len(self.points) < 3:
            msg = "Polygon must have at least three points"
...
class NGon(Polygon):
    def __init__(
        ...
        **kwargs,
        ...
        for i in range(n):
            angle = 2 * np.pi * i / n
            point = center_array + radius * np.array([np.cos(angle), np.sin(angle)])
            points.append(point)
