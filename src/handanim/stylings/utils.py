...
from handanim.core.styles import FillStyle, SketchStyle
...
def rotate_points(
    ...
    points_array = np.array(points)  # (n, 2)
    center_array = np.array(center)  # (2, )
    angle = np.pi * degrees / 180
    cos, sin = np.cos(angle), np.sin(angle)
    rotation_matrix = np.array([[cos, -sin], [sin, cos]])  # (2, 2)
    return (points_array - center_array) @ rotation_matrix + center_array
...
def straight_hachure_lines(
    ...
    while len(active_edges) > 0 or len(edges) > 0:
        ...
        if ((step_offset != 1) or (iteration % gap == 0)) and len(active_edges) > 1:
