...
class Drawable:
    ...
    def draw(self) -> OpsSet:
        ...
        """
        msg = f"No method for drawing {self.__class__.__name__}"
...
class TransformedDrawable(Drawable):
    ...
    def __init__(
        ...
        transformation_function: collections.abc.Callable,
    ...
    def draw(self) -> OpsSet:
        ...
        if not hasattr(opsset, self.transformation_function):
            msg = f"Invalid transformation function {self.transformation_function}"
            raise ValueError(msg)
        if not callable(getattr(opsset, self.transformation_function)):
            msg = f"Transformation function {self.transformation_function} is not callable"
            raise ValueError(msg)
...
class DrawableFill:
    ...
    def fill(self) -> OpsSet:
        msg = "fill method not implemented for base fill pattern"
        raise NotImplementedError(msg)
...
class DrawableCache:
    ...
    def get_cachekey(self, drawable_id: str, event_id: str | None = None) -> str:
        if event_id is None:
            return f"{drawable_id}__init"
        return f"{drawable_id}__{event_id}"

    def set_drawable_opsset(self, drawable: Drawable, opsset: OpsSet | None = None) -> None:
    ...
    def set_drawable_event_opsset(self, drawable_id: str, event_id: str, opsset: OpsSet) -> None:
...
class DrawableGroup(Drawable):
    ...
    def __init__(self, elements: list[Drawable], grouping_method="parallel", *args, **kwargs) -> None:
